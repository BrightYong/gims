...
export class OrdersComponent implements AfterViewInit {
  displayedColumns: string[] = ['id', 'productName', 'quantityOrdered', 'orderPrice', 'totalOrderAmount', 'action'];
  ...
  openSupplyModal(orderItem: IOrder) {
    this.dialog.open(AddSupplyDialog, {
      data: {
        orderItem
      },
      width: '500px'
...
export class AddOrderDialog {
  ...
  onChange(target: any) {
    let selected = productList.find((t) => { return t.ID === target; });
    this.newProduct.price = selected ? selected.amount : 0;
  }

  createOrder() {
    console.log(this.newProduct);
    this.dialogRef.close();
    this.ut.openSnackBar('Order created and sent to suppliers successfully', 'Close')
  }

}

@Component({
  selector: 'dialog-add-supply',
  templateUrl: 'add-supply.html',
})
export class AddSupplyDialog {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any,
    public dialogRef: MatDialogRef<AddOrderDialog>,
    public ut: UtilitiesService) {
      this.newSupplyEntry.quantityOrdered = data.orderItem.quantityOrdered;
      this.newSupplyEntry.quantityDelivered = data.orderItem.quantityDelivered;
  }

  onChange(target: any) {
    let selected = productList.find((t) => { return t.ID === target; });
    this.newSupplyEntry.price = selected ? selected.amount : 0;
  }

  createSupply() {
    console.log(this.newSupplyEntry);
    this.dialogRef.close();
    this.ut.openSnackBar('Order created and sent to suppliers successfully', 'Close')
  }
